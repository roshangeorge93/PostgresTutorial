...
select p.cat_name as parent_cat,c.cat_name from category c inner join category p on c.parent_id=p.cat_id;
...
SELECT p.cat_name AS parent_cat,c.cat_name AS sub_cat 
FROM category c,category p 
WHERE c.parent_id=p.cat_id and p.cat_name='electronics'